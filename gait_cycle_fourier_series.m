...
%% CALCULATIONS
%%  POSITIONS
rib_x_f = fit_x(rib_x,t,8,'rib_x.txt');
rib_y_f = fit_y(rib_y,t,8,'rib_y.txt');

hip_x_f = fit_x(hip_x,t,8,'hip_x.txt');
hip_y_f = fit_y(hip_y,t,8,'hip_y.txt');

knee_x_f = fit_x(knee_x,t,8,'knee_x.txt');
knee_y_f = fit_y(knee_y,t,15,'knee_y.txt');

fibula_x_f = fit_x(fibula_x,t,8,'fibula_x.txt');
fibula_y_f = fit_y(fibula_y,t,15,'fibula_y.txt');

ankle_x_f = fit_x(ankle_x,t,15,'ankle_x.txt');
ankle_y_f = fit_y(ankle_y,t,15,'ankle_y.txt');

heel_x_f = fit_x(heel_x,t,15,'heel_x.txt');
heel_y_f = fit_y(heel_y,t,15,'heel_y.txt');

metat_x_f = fit_x(metat_x,t,15,'metat_x.txt');
metat_y_f = fit_y(metat_y,t,15,'metat_y.txt');

toe_x_f = fit_x(toe_x,t,15,'toe_x.txt');
toe_y_f = fit_y(toe_y,t,15,'toe_y.txt');

%%  ANGLES
foot_theta_f = fit_y(foot_theta,t,15,'foot_theta.txt');     % degree
leg_theta_f = fit_y(leg_theta,t,15,'leg_theta.txt');     % degree
thigh_theta_f = fit_y(thigh_theta,t,15,'thigh_theta.txt');   % degree
HAT_theta_f = fit_y(HAT_theta,t,15,'HAT_theta.txt');     % degree
% A4
hip_theta_f = fit_y(hip_theta,t,15,'hip_theta.txt');    % degree
knee_theta_f = fit_y(knee_theta,t,15,'knee_theta.txt'); % degree
ankle_theta_f = fit_y(ankle_theta,t,15,'ankle_theta.txt'); % degree
%% FORCES AND MOMENTS
F_foot_ground_x_f = fit_y(F_foot_ground_x,t,17,'F_foot_ground_x.txt');  % N
F_foot_ground_y_f = fit_y(F_foot_ground_y,t,17,'F_foot_ground_y.txt');  % N
